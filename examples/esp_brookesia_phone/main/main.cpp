...
extern "C" void app_main(void)
{
    ...
    phone_stylesheet->home.navigation_bar.visual_mode = ESP_BROOKESIA_NAVIGATION_BAR_VISUAL_MODE_SHOW_FLEX;
    ...
    AppEcho *app_echo = new AppEcho();
    assert(app_echo != nullptr && "Failed to create app_echo");
    assert((phone->installApp(app_echo) >= 0) && "Failed to begin app_echo");

    AppHidHost *app_hid_host = new AppHidHost();
    assert(app_hid_host != nullptr && "Failed to create app_hid_host");
    assert((phone->installApp(app_hid_host) >= 0) && "Failed to begin app_hid_host");

#if CONFIG_EXAMPLE_ENABLE_SD_CARD
    AppRecord *app_record = new AppRecord();
    assert(app_record != nullptr && "Failed to create app_record");
    assert((phone->installApp(app_record) >= 0) && "Failed to begin app_record");
#endif
